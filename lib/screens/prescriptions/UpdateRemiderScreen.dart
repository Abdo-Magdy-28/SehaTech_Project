...
class UpdateReminderScreen extends StatefulWidget {
  ...
  UpdateReminderScreen({super.key, required this.reminder});
...
class _UpdateReminderScreenState extends State<UpdateReminderScreen> {
  ...
  void _submitReminder() async {
    ...
    final medName = widget.reminder.medicationName.isNotEmpty
        ? widget.reminder.medicationName
        : _nameController.text.trim().split(' ').first;

    final generic = widget.reminder.genericName.isNotEmpty
        ? widget.reminder.genericName
        : _nameController.text.trim().split(' ').first;
    ...
    final reminder = MedicationReminder(
      ...
      doseTimes: _selectedTimes
          .map(
            (t) => DoseTime(time: t, dosage: '1 ${backendType.toLowerCase()}'),
          )
          .toList(),
  ...
  void initState() {
    ...
    // ── Pre-fill selected times ──
    _selectedTimes = widget.reminder.doseTimes.map((d) => d.time).toList();
    _initialDoseCount = widget.reminder.doseTimes.length;

    // ── Pre-fill strength/dosage unit ──
    final strengthVal = widget.reminder.strength; // e.g. "500mg"
    final normalized = strengthVal.replaceAll('mg', 'gm'); // "500gm"
    _dosageUnit = _kDosageUnits.contains(normalized)
        ? normalized
        : _kDosageUnits.first;

    // ── Pre-fill start date ──
    final start =
        DateTime.tryParse(widget.reminder.startDate) ?? DateTime.now();
    _fromDay = start.day;
    _fromMonth = _kMonths[start.month - 1];
    _fromYear = start.year;

    // ── Pre-fill end date ──
    final end = DateTime.tryParse(widget.reminder.endDate) ?? DateTime.now();
    _toDay = end.day;
    _toMonth = _kMonths[end.month - 1];
    _toYear = end.year;

    // ── Pre-fill form (syrup/capsule) ──
    _initialForm = widget.reminder.form.toLowerCase(); // "syrup" or "capsule"

    _initialDoseCount = widget.reminder.doseTimes.length;
  }

  late int _initialDoseCount;
  late String _initialForm;
  @override
  @override
  @override
  void didChangeDependencies() {
    ...
    if (!_labelsInitialized) {
      ...
      // ── match form to localized label ──
      _selectedTypeLabel = _initialForm == 'syrup'
          ? s.reminderTypeSyrup
          : s.reminderTypeCapsule;

      // ── match frequency to dose count ──
      if (_initialDoseCount == 2) {
        _freqTimesLabel = s.reminderFreqTwice;
      } else if (_initialDoseCount >= 3) {
        _freqTimesLabel = s.reminderFreqThreeTimes;
      } else {
  ...
  Widget build(BuildContext context) {
    final takenTimes = widget.reminder.doseTimes
        .where((d) => d.taken)
        .map((d) => d.time)
        .toList();
    final s = S.of(context);
        ...
        body: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              // extract taken times from reminder doseTimes

              // then in your Column:
              _TimeGrid(
                times: _kTimes,
                selected: _selectedTimes,
                takenTimes: takenTimes, // ← new
                sw: sw,
                sh: sh,
                onTap: (t) {
                  // prevent toggling already-taken times
                  if (takenTimes.contains(t)) return;
                  setState(() {
...
class _TimeGrid extends StatelessWidget {
  ...
  final List<String> takenTimes; // ← new
  ...
  Widget build(BuildContext context) => GridView.builder(
    ...
    itemBuilder: (_, i) {
      ...
      final taken = takenTimes.contains(t); // ← new
        ...
        child: Stack(
          children: [
            Container(
              decoration: BoxDecoration(
                color: taken
                    ? const Color(0xFF059669) // green = taken
                    : sel
                    ? const Color(0xFF2260FF) // blue = selected
                    : const Color(0xFFEEF2FF), // grey = unselected
              ...
              child: Center(
                child: Text(
                  ...
                  style: TextStyle(
                    ...
                    fontFamily: 'Cairo',
                    fontWeight: FontWeight.w600,
                    color: (sel || taken) ? Colors.white : Colors.black87,
                  ),
                ),
              ),
            ),
            // ── Taken checkmark badge ──
            if (taken)
              Positioned(
                top: 2,
                right: 4,
